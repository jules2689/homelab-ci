"""GitHub App auth: JWT and installation access tokens for Checks API and repo access."""
import logging
import os
import time
import json
...
# Cache: (owner, repo) -> (installation_id,); installation_id -> (token, expires_at)
...
def _load_private_key_pem() -> str:
    ...
    # Private key PEM (the .pem file contents). Not the app "client secret".
    key = os.environ.get("GITHUB_APP_PRIVATE_KEY")
    if key:
        pem = key.replace("\\n", "\n").strip()
        logger.debug("Using key from GITHUB_APP_PRIVATE_KEY (%d chars)", len(pem))
        return pem
    # Path to .pem file: GITHUB_APP_KEY_PATH or GITHUB_APP_PRIVATE_KEY_PATH
...
def _load_private_key():
    """Load PEM and return a cryptography key object so PyJWT can sign reliably."""
    ...
    if "-----BEGIN" not in pem or "PRIVATE KEY" not in pem:
        ...
        logger.error(
            "GitHub App key invalid: got %d chars, starts with: %r",
            len(pem),
            preview,
        )
        raise ValueError(
            "GitHub App private key must be PEM (-----BEGIN ... PRIVATE KEY-----). "
            "Got %d chars starting with %r. Check GITHUB_APP_PRIVATE_KEY or GITHUB_APP_KEY_PATH."
            % (len(pem), preview)
        )
    try:
...
def get_installation_token_for_repo(owner: str, repo: str) -> str:
    """Return an installation access token for the given repo. Cached and refreshed as needed."""
...
def is_github_app_configured() -> bool:
    """True if GITHUB_APP_ID and private key (GITHUB_APP_PRIVATE_KEY or GITHUB_APP_KEY_PATH) are set."""
